...
async function disegnaFrontespizio(
  ...
  centrato(
    page,
    helveticaBold,
    `UFFICIO DELLE DOGANE DI ${input.ufficioDogane.toUpperCase()}`,
    y,
    13,
  );
  y -= 28;
  ...
  centrato(
    page,
    helvetica,
    `sita in ${input.indirizzo} ${input.cap} ${input.comune} (${input.provincia})`,
    y,
    10,
  );
  y -= 26;
  ...
  centrato(
    page,
    helveticaBold,
    "Registro delle letture dei contatori elettrici",
    y,
    12,
  );
  y -= 18;
...
function disegnaPaginaAnno(
  ...
  page.drawText(serieTesto, {
    x: PAGE_WIDTH - MARGIN - serieWidth,
    y: y - 10,
    size: 8,
    font: helvetica,
  });
  ...
  centrato(
    page,
    helveticaBold,
    "ACCISA SUL GAS E SULL'ENERGIA ELETTRICA",
    y,
    10,
  );
  y -= 16;
  centrato(
    page,
    helvetica,
    `Ditta ${input.ragioneSociale} - ${input.comune}`,
    y,
    9,
  );
  y -= 14;
  ...
  const larghezzaContatore =
    (larghezzaTabella - larghezzaLabel - larghezzaAnnotazioni) /
    input.contatori.length;
  ...
  for (const x of [
    MARGIN,
    xLabelDivisore,
    xAnnotazioniDivisore,
    MARGIN + larghezzaTabella,
  ]) {
    page.drawLine({
  ...
  // non si può riusare l'helper `centrato`): calcolo manuale.
  {
    ...
    page.drawText(testo1, {
      x: centro - larghezzaTesto1 / 2,
      y: yImpiantoTesto,
      size: 8,
      font: helveticaBold,
    });
  ...
  input.contatori.forEach((c, i) => {
    page.drawText(TIPO_LABEL[c.tipo], {
      x: colonneX[i + 1] + 4,
      y: yMisura,
      size: 8,
      font: helvetica,
  ...
  MESI.forEach((nomeMese, indiceMese) => {
    ...
    page.drawText(`${nomeMese} ${ultimoGiorno}`, {
      x: MARGIN + 4,
      y: yRigaMese,
      size: 8,
      font: helvetica,
  ...
  y -= 30;
  page.drawText("(*) lettura fine mese", {
    x: MARGIN,
    y,
    size: 8,
    font: helvetica,
  ...
  page.drawText(lineaFirma, {
    x: PAGE_WIDTH - MARGIN - larghezzaLineaFirma,
    y,
    size: 9,
    font: helvetica,
  });
  ...
  page.drawText(perLaDitta, {
    x: PAGE_WIDTH - MARGIN - larghezzaPerLaDitta,
    y,
    size: 8,
    font: helvetica,
  });
...
export async function generaRegistroLetturePdf(
  input: RegistroLettureInput,
): Promise<Uint8Array> {
  ...
  const logo = await pdfDoc.embedJpg(
    Buffer.from(LOGO_AGENZIA_DOGANE_BASE64, "base64"),
  );
