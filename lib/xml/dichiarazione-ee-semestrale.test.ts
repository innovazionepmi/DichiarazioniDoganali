...
const INPUT_SINTETICO: DichiarazioneEeSemestraleInput = {
  ...
  quadroA: KWH_PRODUZIONE.map((kwh, i) =>
    meseA(i + 1, lettPProduzione[i], kwh),
  ),
  quadroC: KWH_PRODUZIONE.map((kwh, i) => ({
    numMese: i + 1,
    kwh: kwh - KWH_CESSIONE[i],
  })),
...
describe("generaDichiarazioneEeSemestraleXml", () => {
  ...
  it("scrive il Quadro C tra A e G con Matr segnaposto e Tipologia L2", () => {
    ...
    const totaleC =
      KWH_PRODUZIONE.reduce((a, b) => a + b, 0) -
      KWH_CESSIONE.reduce((a, b) => a + b, 0);
    expect(xml.split("<C>")[1].split("</C>")[0]).toContain(
      `<TotaleQuadro>${totaleC}</TotaleQuadro>`,
    );
  });
  ...
  it("omette il Quadro G quando non c'è cessione alla rete", () => {
    const xml = generaDichiarazioneEeSemestraleXml({
      ...INPUT_SINTETICO,
      quadroG: null,
    });
...
describe("parseDichiarazioneEeSemestraleXml", () => {
  ...
  it("non perde gli zeri iniziali di una matricola tutta numerica", () => {
    // Caso reale (Scuola Provera): matricola "00020234" ricompariva come
    // "20234" nel PDF di ricevuta/anteprima — fast-xml-parser interpreta di
    // default il testo del tag come number quando "sembra" un numero.
    const input: DichiarazioneEeSemestraleInput = {
      ...INPUT_SINTETICO,
      quadroA: INPUT_SINTETICO.quadroA.map((mese, i) =>
        i === 0
          ? {
              ...mese,
              contatori: [{ ...mese.contatori[0], matricola: "00020234" }],
            }
          : mese,
      ),
    };
    const xml = generaDichiarazioneEeSemestraleXml(input);
    expect(xml).toContain("<Matr>00020234</Matr>");
    const parsed = parseDichiarazioneEeSemestraleXml(xml);
    expect(parsed.quadroA[0].contatori[0].matricola).toBe("00020234");
  });
});
