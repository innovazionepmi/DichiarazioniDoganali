...
export type CaricaDocumentoResult =
  { error: string } | { documentoId: string; storagePath: string };
...
// 'excel_letture': il registro letture Excel che Paolo tiene già per ogni
// cliente (parsing deterministico via etichette, lib/parsers/registro-letture-excel.ts).
export async function caricaDocumento(
  ...
  if (!TIPI_CONSENTITI.includes(file.type)) {
    return {
      error:
        "Formato file non supportato: solo PDF, TXT, immagine (PNG/JPEG/WEBP) o Excel (.xlsx).",
    };
  }
...
export type ScaricaDocumentoResult =
  { error: string } | { base64: string; nomeFile: string; mimeType: string };
...
export async function scaricaDocumento(
  documentoId: string,
): Promise<ScaricaDocumentoResult> {
  const supabase = await createClient();
  ...
  if (error || !documento)
    return { error: error?.message ?? "Documento non trovato" };
